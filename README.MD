...
*   **Live Chat**: Real-time messaging for employees to collaborate.
...
*   **Database (MongoDB)**: Uses **MongoDB Atlas** (External) for production-grade, cloud-managed data persistence.
...
*   **CI/CD**: **Jenkins** automates the entire pipeline: Build -> Push to DockerHub -> Deploy to Kubernetes via Helm.
*   **Monitoring**: **Prometheus** scrapes metrics from the cluster, while **Grafana** visualizes system health.
...
*   **`Install Build Tools`**:
    Run these commands in your terminal to install AWS CLI and Helm inside the Jenkins container:
    ```bash
    docker exec -u root -it jenkins bash -c "apt-get update && apt-get install -y unzip && curl 'https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip' -o 'awscliv2.zip' && unzip awscliv2.zip && ./aws/install"
    docker exec -u root -it jenkins bash -c "curl -fsSL -o get_helm.sh https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 && chmod 700 get_helm.sh && ./get_helm.sh"
    ```
...
Manage Jenkins > Credentials > System > Global credentials > Add Credentials:
   ...
   *   **`flask-secret-key`**: *Secret text* (A strong random string, e.g., `openssl rand -hex 32`).
...
Deploy the Monitoring stack using raw manifests.
...
```bash
# 1. Create Secrets (Manual Deployment Only)
# Copy the template and fill in your credentials (this file is ignored by git)
cp devops-infra/helm/backend/secrets.template.yaml devops-infra/helm/backend/secrets.yaml
# Edit devops-infra/helm/backend/secrets.yaml with your real values now

# Install Backend
helm upgrade --install office-backend ./devops-infra/helm/backend -f devops-infra/helm/backend/secrets.yaml
...
Create 2 separate Pipeline jobs in Jenkins.
...
#### 4. Troubleshooting: Triggers on Localhost
If Jenkins is running locally (Docker Desktop), GitHub cannot send Webhooks to `localhost`.
**Workaround:** Configure Jenkins to poll GitHub for changes.
1.  Go to your **office-frontend** and **office-backend** jobs.
2.  Click **Configure**.
